...
public class UpdateUser extends BaseApi {
    ...
    private String endpointValid() { return baseUrl() + "user/{idUserNew}"; }
    private String endpointInvalid() { return baseUrl() + "user/0"; }
    private String endpointNull() { return baseUrl() + "user/null"; }

    @Step("I set an endpoint for update user")
    ...
    @Step("I request {0} for update detail user")
    public void requestUpdateFieldDetailUser(String field) {
        switch (field) {
    ...
    @Step("I validate the status code for update user is {0}")
    public void validateTheStatusCodeForUpdateDataUser(int statusCode) { validateStatusCode(statusCode); }

    @Step("validate the data detail after update user")
    public void validateTheDataDetailAfterUpdateUser(String message) {
        switch (message) {
            case "success":
                TestDataStore.set(TestDataStore.ID_USER_UPDATE, getResponseValue("data.id"));
    ...
    private void putNull() {
        JSONObject body = buildBody("username", null, "password", null);
        String id = TestDataStore.get(TestDataStore.ID_USER_NEW);
        adminRequest().pathParam("idUserNew", id).body(body.toJSONString()).put(endpointValid());
    }
}
